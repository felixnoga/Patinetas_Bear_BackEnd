...
class Client extends User {
  constructor(
    ...
    no_trips = null,
    app_uses = null,
    mapbox_token = secrets.mapboxtoken
  ) {
    super();
...
class ClientManager extends UserManager {
  ...
  static async getOnlyClient(client_id) {
    const BDClient = startConnection();
    const queryresponse = await BDClient.query(
  ...
  static async updateClientBalance({ client_id, amount }) {
    ...
    const query =
      'UPDATE clients SET balance = balance + $1 WHERE client_id = $2 RETURNING *';
    try {
      const queryresponse = await BDClient.query(query, [
        convertedAmount,
        client_id,
      ]);
  ...
  static async registerClient(newUser) {
    ...
    try {
      const data = await BDClient.query(
        `SELECT * FROM clients WHERE client_id= $1`,
        [client_id]
      );
      const arr = data.rows;
      ...
      if (arr.length != 0) {
        ...
        const newClient = await BDClient.query(
          `INSERT INTO clients (client_id, balance, no_trips, app_uses, mapbox_token) VALUES ($1,$2,$3,$4,$5) RETURNING *;`,
          [
            client.client_id,
            client.balance,
            client.no_trips,
